...
export const CharacterDetails = () => {
  const { id } = useParams()
  const { data, loading, error } = useGetData(`${VITE_API_URL}/characters/${id}`, 'character')
        ...
        <section className="max-w-screen-xl mx-auto px-4 py-8">
          <article className="md:flex md:gap-x-8 ">
            <picture className="rounded-lg overflow-hidden w-96 h-96">
              <img src={data?.image} alt={data?.name} width={300} height={500} className="object-cover w-full h-full" />
            </picture>
            <aside>
